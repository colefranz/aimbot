(function(angular) {
	"use strict";
	angular.module("aimbot", ["ngRoute"]).config([
		"$routeProvider",
		"$locationProvider",
		...
		[
			...
			"Timer",
			"configConstants",
			function($scope, $document, $location, Timer, configConstants) {
				var customTps;
				var timer;
				$scope.gameState = {
					gameActive: false,
				...
				$scope.commands = {
					...
					togglePauseGame: function() {
						$scope.gameState.gamePaused
							? $scope.commands.unpauseGame()
							: $scope.commands.pauseGame();
					},
