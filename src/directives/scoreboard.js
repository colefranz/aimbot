...
.directive('scoreboard', ['$filter', '$timeout', function($filter, $timeout) {
  return {
    restrict: 'E',
    link: function(scope) {
      var setAccuracy;

      scope.$watch('gameStats.score.value', function(targetsHit) {
        $timeout(function() {
      ...
      scope.$watch('gameStats.targetsMissed.value', function(targetsMissed) {
        $timeout(function() {
          setAccuracy(scope.gameStats.score.value / targetsMissed * 100);
        }, 0);
      });
